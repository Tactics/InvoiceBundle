...
class InvoiceConverter
{
    ...
    private function getProAccVerkoopLijnen(Invoice $invoice, $options)
    {
        ...
        foreach ($invoice->getItems() as $cnt => $item) {
            ...
            $line = array_merge($blancos, array(
                ...
                'AG' => substr(preg_replace('/[ \t]+/', ' ', $item->getDescription()), 0, 50), // omschrijving, voor inovant moet hier de opleidingscode inkomen
     ...
     */
    protected function getBoekingsperiode(Invoice $invoice): string
    {
    ...
    private function getProAccFacturatieLijnen(Invoice $invoice, $options)
    {
        ...
        foreach ($invoice->getItems() as $cnt => $item) {
            ...
            $line = array(
                ...
                'S' => preg_replace('/[ \t]+/', ' ', $item->getDescription()), // max 300
                'T' => $item->getQuantity(),
